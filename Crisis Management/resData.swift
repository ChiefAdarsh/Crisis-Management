...
class Resource{
    ...
    func toArray(){
        if category == "Crisis Lines"{
            print(category + ", " + name + "; " + contact + "; " + website + "; " + addInfo)
        }
        else{
            print(category + ", " + name + "; " + address + city + ", " + state + " " + zip + "; " + contact + "; " + website + "; " + addInfo)
        }
...
func createResources() -> [Resource] {
    ...
    myArray.append(res)
    res = Resource(category: "Crisis Lines", name: "Suicide Crisis Center of North Texas", contact: "214-828-1000", address: "", city: "", state: "", zip: "", website: "", addInfo: "Suicide and Crisis Hotline; Available 24/7 for anyone in crisis or contemplating suicide; Programs for those who have lost a loved one to suicide (free); Teen Programs (TeenScreen) (free)")
    myArray.append(res)
    res = Resource(category: "Crisis Lines", name: "Texas Youth and Runaway Hotline", contact: "Hotline: 1-800-989-6884; Texting - 512-872-5777; Chat - texasyouth.org", address: "", city: "", state: "", zip: "", website: "texasyouth.org", addInfo: "Individuals can call, chat or text; Crisis Intervention; Referrals for callers who need shelter, counseling, and transportation home; A confidential message relay service is available to promote communications between runaways and their families")
    myArray.append(res)
    res = Resource(category: "Crisis Lines", name: "Love is Respect", contact: "Hotline: 1-866-331-9474; Texting - love is to 22522; Chat - loveisrespect.org", address: "", city: "", state: "", zip: "", website: "loveisrespect.org", addInfo: "24 hour web based and telephone helpline from National Domestic Violence Hotline; Created to help teens (ages 13-18) to prevent and end dating abuse")
    myArray.append(res)
    res = Resource(category: "Crisis Lines", name: "The Trevor Project", contact: "1-866-488-7386", address: "", city: "", state: "", zip: "", website: "thetrevorproject.org", addInfo: "Hotline 24/7; Individuals can call, chat, or text; Crisis Intervention and suicide prevention services for lesbian, gay, bisexual, transgender and questioning (LGBTQ) youth under age 25")
    myArray.append(res)
    res = Resource(category: "Crisis Lines", name: "Here for Texas", contact: "972-525-8181", address: "", city: "", state: "", zip: "", website: "www.herefortexas.com", addInfo: "Free Service; Connects individuals to a trained mental health navigator who can offer support and information about mental health services in our area; Resources are available for anxiety, depression, bipolar, addictions, and other mental health conditions; Helps with transportation to the mental health facility")
    myArray.append(res)
    ...
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "Coppell Counseling Center", contact: "972-393-1596", address: "413 W. Bethel Road, Suite 100", city: "Coppell", state: "Texas", zip: "75019", website: "www.coppellcounseling.com", addInfo: "A suite of independent licensed mental health practitioners; Coppell Counseling Center mental health professionals participate in many preferred provider organizations.  ")
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "Dallas DBT", contact: "972-918-0170", address: "9900 North Central Expressway, Suite 500", city: "Dallas", state: "Texas", zip: "75231", website: "www.dallasdbt.com", addInfo: "Offers tele-health; Focus: Team of clinicians practicing dialectical behavioral therapy through individual therapy, skills groups, coaching calls, and clinician consultations; Age: Children, adolescents, adults, and families; Insurance: They do not accept insurance. See website for rates.  ")
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "Family Tree", contact: "972-353-9404; 888-837-0666, ext. 7 (Dallas County)", address: "2727 LBJ Freeway, Suite 406", city: "Farmers Branch", state: "Texas", zip: "75234", website: "www.familytreeprogram.org", addInfo: "Offers tele-health; Family counseling at no cost.; Spanish speaking available; Families are usually eligible if they have a youth 17 or younger in the home, they currently are not receiving services from CPS, and the youth has never been adjudicated of a crime.; Presenting issues can be served in the short term services provided.")
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "Holiner Psychiatric Group", contact: "972-566-4591", address: "7777 Forest Lane, Building C, Suite 833", city: "Dallas", state: "Texas", zip: "75230", website: "www.holinergroup.com", addInfo: "Offers tele-health; Offers tele-health; Focus: A full service practice committed to providing high quality psychiatric services.; Age: Children, adolescents and adults.; Insurance: See their website for a list of insurance companies or call to verify.; They accept Medicare, Value Options.")
    myArray.append(res)
    ...
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "Richland Oaks Counseling Center", contact: "469-619-7622", address: "1221 Abrams Rd., Suite 325", city: "Richardson", state: "Texas", zip: "75081", website: "www.richlandoaks.org", addInfo: "Offers tele-health; A nonprofit teaching and research clinic with locations in Richardson, Plano, McKinney and Prosper; Full range of counseling services and psychological assessment; Bilingual services in: Spanish, Urdu, Punjabi, Hindi, and conversational ASL; Insurance: They accept many major insurance groups, including Medicare. Call for availability.")
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "SMU Center for Family Counseling", contact: "214-768-6789", address: "6116 N. Central Expressway #410", city: "Dallas", state: "Texas", zip: "75206", website: "www.smu.com/familycounseling", addInfo: "In-person and telehealth low-cost counseling for children, adolescents and families; Monday through Thursday from 11:30am-7:30pm")
    myArray.append(res)
    res = Resource(category: "Counseling Centers", name: "Taylor Counseling Group", contact: "214-530-0021", address: "870 S. Denton Tap Road, Suite 150", city: "Coppell", state: "Texas", zip: "75019", website: "taylorcounselinggroup.com", addInfo: "Offers tele-health; Practice of clinicians that provide individual, family and play therapy for ages 5-adult; Insurance: They are in-network with most major insurance groups. They also offer an affordable care program for reduced fees.")
    myArray.append(res)
    ...
    myArray.append(res)
    res = Resource(category: "Eating Disorders", name: "Elisa Project", contact: "866-837-1999; 214-369-5222", address: "10300 N. Central Expressway, #330", city: "Dallas", state: "Texas", zip: "75231", website: "www.theelisaproject.org", addInfo: "Focus: Fighting against eating disorders through education, support, and advocacy.  The ELISA project is not a treatment center; Connect families and individuals with resources.; Age: Resources for all ages, from young children to adults.; Cost: Free of charge")
    myArray.append(res)
